...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.port_Status.setText("Port: %s Phase: %i L: %s R: %s"% (prep.portName, prep.trainingPhase,format(prep.Ltime, '1.2f'),format(prep.Rtime, '1.2f')) )
##        self.db_status.setText('    SQL(%s): %s'% (prep.sql_status,dbName))
##        self.db_status.setText('    SQL(%s)'% (prep.sql_status))
...
######        
        self.overallCorrect = QtWidgets.QLabel(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Expanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.noText.sizePolicy().hasHeightForWidth())
        self.overallCorrect.setSizePolicy(sizePolicy)
        self.overallCorrect.setMinimumSize(QtCore.QSize(0, 30))
        self.overallCorrect.setMaximumSize(QtCore.QSize(16777215, 40))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.overallCorrect.setFont(font)
        self.overallCorrect.setAlignment(QtCore.Qt.AlignCenter)
        self.overallCorrect.setObjectName("overallCorrect")
        self.gridLayout_3.addWidget(self.overallCorrect, 1, 6, 1, 1)

        self.overallCorrectLabel = QtWidgets.QLabel(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Expanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.overallCorrectLabel.sizePolicy().hasHeightForWidth())
        self.overallCorrectLabel.setSizePolicy(sizePolicy)
        self.overallCorrectLabel.setMinimumSize(QtCore.QSize(0, 30))
        self.overallCorrectLabel.setMaximumSize(QtCore.QSize(16777215, 40))
        font = QtGui.QFont()
        font.setPointSize(10)
        self.overallCorrectLabel.setFont(font)
        self.overallCorrectLabel.setObjectName("overallCorrectLabel")
        self.gridLayout_3.addWidget(self.overallCorrectLabel, 0, 6, 1, 1)

        self.linezz = QtWidgets.QFrame(self.centralwidget)
        self.linezz.setFrameShape(QtWidgets.QFrame.VLine)
        self.linezz.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.linezz.setObjectName("linezz")
        self.gridLayout_3.addWidget(self.linezz, 0, 5, 1, 1)

        self.linez = QtWidgets.QFrame(self.centralwidget)
        self.linez.setFrameShape(QtWidgets.QFrame.VLine)
        self.linez.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.linez.setObjectName("linez")
        self.gridLayout_3.addWidget(self.linez, 1, 5, 1, 1)
######        
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.overallCorrectLabel.setText(_translate("MainWindow", "% ovrCorrect"))
        ...
        self.overallCorrect.setText(_translate("MainWindow", "ovrCorrectText"))
    ...
    def update_figure(self):
        ...
        if not new_stuff.value:
            ...
            self.automatic = dump[3]
            
            if self.automatic == 0 or prep.trainingPhase == 0 or prep.trainingPhase == 10 :
                self.trialsNoFree +=1
                ...
                try:
                    ...
                    self.ntargetText.setText(str(self.trialsNoFree - self.totalTarget))
                    self.trailText.setText(str(self.trialsNoFree))
                    ...
                    try:
                        ...
                        self.ntargetCorrect.setText("{0:.2f}".format(self.intargetCorrect / (self.trialsNoFree - self.totalTarget) * 100)+'%')
                    ...
                    try:
                        print(self.intargetCorrect)
                        self.ntargetWrong.setText("{0:.2f}".format(self.intargetWrong / (self.trialsNoFree - self.totalTarget) * 100)+'%')

                    except Exception as e:
                        print(str(e))
                        self.ntargetWrong.setText('0 %')

                    self.correctText.setText("{0:.2f}".format(self.totalCorrect/self.trialsNoFree*100))
                    chinita += 1
                    self.wrongText.setText("{0:.2f}".format(self.totalWrong/self.trialsNoFree*100))
                    chinita += 1
                    self.noText.setText("{0:.2f}".format(self.totalNo/self.trialsNoFree*100))
                    chinita += 1
                    self.overallCorrect.setText("{0:.2f}".format(self.totalCorrect/(self.totalCorrect+self.totalWrong)*100))
                    chinita += 1
    ...
    def writeSQL(self):
        ...
        self.cursor.execute(
            "INSERT INTO  temporal_trails ( Session_ID, Trail_ID, SequenceStartTime, Song, LickTime, Difficulty, Correctness, LickResult,AssistDrop) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (self.session_ID, self.ind, str(t_zero), self.song_mem[0], sevent_time, difficulty, correct,direction,automatic))
        self.cnx.commit()
    ...
    def writeCSV(self):
        ...
        difficulty = dump[2]
        automatic = dump[3]
        self.writer.writerow([self.session_ID, self.ind, str(t_zero), self.song_mem[0], str(event_time), difficulty, correct,direction,automatic])
